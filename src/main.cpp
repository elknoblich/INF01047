...
int main() {
   ...
   float speed                 = 0.5;
   glm::vec4 camera_position_c = glm::vec4(-0.75f, 0.75f, 0.75f, 1.0f); // Ponto "c", centro da câmera
   ...
   while (!glfwWindowShouldClose(window)) {
      ...
      for (int i = 1; i <= 3; ++i) {
         ...
         glDrawElements(g_VirtualScene["cube_faces"].rendering_mode, // Veja slides 182-188 do documento Aula_04_Modelagem_Geometrica_3D.pdf
                        g_VirtualScene["cube_faces"].num_indices, GL_UNSIGNED_INT, (void *)g_VirtualScene["cube_faces"].first_index);
