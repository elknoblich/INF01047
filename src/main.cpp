...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - 00279050 - Nicolas Andre Nunes da Silva", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        /*CABEÇA*/
        PushMatrix(model); // Insere Coordenadas do torso
            ...
            model = model * Matrix_Rotate_Z(g_AngleZ)  * Matrix_Rotate_Y(-g_AngleY)  * Matrix_Rotate_X(-g_AngleX);
            model = model * Matrix_Scale(-0.35f, -0.35f, 0.2f);
            glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
            DrawCube(render_as_black_uniform); // CABEÇA
        PopMatrix(model); // Retira coordenada do torso

        /** BRAÇO - ANTEBRAÇO - MÃO(Direito)**/
        PushMatrix(model); // Guardamos COORDEANADAS DO TORÇO
            ...
            PushMatrix(model); // GURADADO COORDENADAS DO BRAÇO
                ...
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    ...
                    PopMatrix(model);
                PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
            PopMatrix(model); // VOLTA MATRIZ COORDENADAS DO BRAÇO
        PopMatrix(model); //VOLTA COORDENADAS DO TORÇO

        /** BRAÇO - ANTEBRAÇO - MÃO(esquerdo)**/
        PushMatrix(model); // Guardamos COORDEANADAS DO TORÇO
            model = model * Matrix_Translate(0.55f, 0.0f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com uma translação para o braço direito
            PushMatrix(model); // GURADADO COORDENADAS DO BRAÇO
                model = model // Atualizamos matriz model (multiplicação à direita) com a rotação do braço direito
        ...
        PopMatrix(model); //VOLTA COORDENADAS DO TORÇO
                          //
        /*PERNA - PERNA -PÈ - DIREITO*/

        PushMatrix(model);
            model = model * Matrix_Translate(-0.2f, -1.05f, 0.0f);
            PushMatrix(model);
                model = model * Matrix_Scale(0.3f, 0.7f, 0.27f); // Atualizamos matriz model (multiplicação à direita) com um escalamento do antebraço direito
                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                DrawCube(render_as_black_uniform); // #### ANTEBRAÇO DIREITO // Desenhamos o antebraço direito
            PopMatrix(model);
            PushMatrix(model);
                ...
                PopMatrix(model);
                PushMatrix(model);
                    model = model * Matrix_Translate(0.0f, -0.75f, 0.15f);
                    model = model * Matrix_Scale(0.25f, 0.1f, 0.5f); // Atualizamos matriz model (multiplicação à direita) com um escalamento do antebraço direito
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                    DrawCube(render_as_black_uniform); // #### ANTEBRAÇO DIREITO // Desenhamos o antebraço direito
                PopMatrix(model);
            PopMatrix(model);
        PopMatrix(model);
        /*PERNA - PERNA -PÈ - esquerdo*/
